...
public class ExpressionTest {
  ...
  public void testDifferentiateMultiply() {
    Expression exp = new Operation(
      '+',
      new Operation('*', one, one),
      new Operation('*', x, zero)
    );
    assertEquals(
      "expected differentiated expression",
      exp2.differentiate("x"),
      exp
    );
  }
  ...
  public void testDifferentiateSingleSameVariable() {
    Expression left = new Operation(
      '*',
      new Operation('+', zero, one),
      new Operation('*', x, one)
    );
    Expression right = new Operation(
      '*',
      new Operation('+', one, x),
      new Operation(
        '+',
